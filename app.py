...
def render_step_item(index, step):
    ...
    if isinstance(step, dict):
        ...
        if step_type == "table":
            ...
            st.text(f"{index}. {step.get('content', '')}")
    else:
...
def main():
    ...
    with tab_caesar:
        render_caesar_tab() # (Fungsi ini ada di kode app.py versi sebelumnya)

    with tab_vigenere:
        render_vigenere_tab() # (Fungsi ini ada di kode app.py versi sebelumnya)
